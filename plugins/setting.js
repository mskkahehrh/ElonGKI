...
const { input, get, updb, updfb, inputForSession, getSessionConfigSnapshot } = require("../lib/database")

// =========================================================
// HELPER: update a setting for a specific session only,
// then persist to Mongo so it survives restarts.
// Falls back to the old global input/updb if no sessionNumber.
// =========================================================
async function inputSession(sessionNumber, setting, data, setUserConfigInMongo) {
  if (sessionNumber && setUserConfigInMongo) {
    await inputForSession(sessionNumber, setting, data);
    const snapshot = getSessionConfigSnapshot(sessionNumber);
    await setUserConfigInMongo(sessionNumber, snapshot);
  } else {
    // Legacy fallback (should not normally be reached)
    await input(setting, data);
    await updb();
  }
}
...
async (conn, mek, m, { from, isOwner, senderNumber, botNumber, reply, config: cfg, sessionNumber }) => {
    try {
        if (!isAllowed(isOwner, senderNumber, botNumber))
            return reply("*⛔ Only the bot owner can use this command!*")

        // Use the per-session config if available, fall back to global config
        const { getSessionConfig } = require('../lib/database')
        const sessionCfg = sessionNumber ? getSessionConfig(sessionNumber) : null
        const activeCfg = sessionCfg || cfg || config

        const prefix = activeCfg.PREFIX || config.PREFIX || '.'

        const workType   = (activeCfg.WORK_TYPE        || config.WORK_TYPE        || 'private').toLowerCase()
        const autoView   = (activeCfg.AUTO_VIEW_STATUS  || config.AUTO_VIEW_STATUS  || 'false').toLowerCase()
        const autoLike   = (activeCfg.AUTO_LIKE_STATUS  || config.AUTO_LIKE_STATUS  || 'false').toLowerCase()
        const autoRec    = (activeCfg.AUTO_RECORDING    || config.AUTO_RECORDING    || 'false').toLowerCase()
        const autoReply  = (activeCfg.AUTO_REPLY        || config.AUTO_REPLY        || 'false').toLowerCase()
        const autoAI     = (activeCfg.AUTO_AI           || config.AUTO_AI           || 'false').toLowerCase()
        const autoTyping = (activeCfg.AUTO_TYPING       || config.AUTO_TYPING       || 'false').toLowerCase()
        const ownerReact = (activeCfg.OWNER_REACT       || config.OWNER_REACT       || 'true').toLowerCase()
        const buttonMode = (activeCfg.BUTTON            || config.BUTTON            || 'true').toLowerCase()
        const alwaysOff  = (activeCfg.ALLWAYS_OFFLINE   || config.ALLWAYS_OFFLINE   || 'false').toLowerCase()
        const lang       = (activeCfg.LANG              || config.LANG              || 'EN').toUpperCase()
...
async (conn, mek, m, { isOwner, senderNumber, botNumber, reply, sessionNumber, setUserConfigInMongo }) => {
    try {
        if (!isAllowed(isOwner, senderNumber, botNumber))
            return reply("*⛔ Only the bot owner can use this command!*")
        await updfb()
        await updb()
        // Also clear this session's in-memory config so it reloads from defaults
        if (sessionNumber && setUserConfigInMongo) {
            await setUserConfigInMongo(sessionNumber, {})
        }
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, sessionNumber, setUserConfigInMongo }) => {
    try {
        if (!isAllowed(isOwner, senderNumber, botNumber))
            return reply("*⛔ Only the bot owner can use this command!*")
        ...
        await inputSession(sessionNumber, "WORK_TYPE", val, setUserConfigInMongo)
...
// =========================================================
// setalways — Always Online ON/OFF
// =========================================================
cmd({
    pattern: "setalways",
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, sessionNumber, setUserConfigInMongo }) => {
    try {
        ...
        await inputSession(sessionNumber, "ALLWAYS_OFFLINE", dbVal, setUserConfigInMongo)
...
// =========================================================
// setautotyping — Fake Typing ON/OFF
// =========================================================
cmd({
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, sessionNumber, setUserConfigInMongo }) => {
    try {
        ...
        await inputSession(sessionNumber, "AUTO_TYPING", dbVal, setUserConfigInMongo)
...
// =========================================================
// setautoview — Auto View Status ON/OFF
// =========================================================
cmd({
    pattern: "setautoview",
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, sessionNumber, setUserConfigInMongo }) => {
    try {
        ...
        const dbVal = val === 'on' ? 'true' : 'false'
        await inputSession(sessionNumber, "AUTO_VIEW_STATUS", dbVal, setUserConfigInMongo)
...
// =========================================================
// setautolike — Auto Like Status ON/OFF
// =========================================================
cmd({
    pattern: "setautolike",
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, sessionNumber, setUserConfigInMongo }) => {
    try {
        ...
        const dbVal = val === 'on' ? 'true' : 'false'
        await inputSession(sessionNumber, "AUTO_LIKE_STATUS", dbVal, setUserConfigInMongo)
...
// =========================================================
// setautorec — Auto Recording ON/OFF
// =========================================================
cmd({
    pattern: "setautorec",
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, sessionNumber, setUserConfigInMongo }) => {
    try {
        ...
        const dbVal = val === 'on' ? 'true' : 'false'
        await inputSession(sessionNumber, "AUTO_RECORDING", dbVal, setUserConfigInMongo)
...
// =========================================================
// setautoreply — Auto Reply ON/OFF
// =========================================================
cmd({
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, sessionNumber, setUserConfigInMongo }) => {
    try {
        ...
        const dbVal = val === 'on' ? 'true' : 'false'
        await inputSession(sessionNumber, "AUTO_REPLY", dbVal, setUserConfigInMongo)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, sessionNumber, setUserConfigInMongo }) => {
    try {
        ...
        const dbVal = val === 'on' ? 'true' : 'false'
        await inputSession(sessionNumber, "AUTO_AI", dbVal, setUserConfigInMongo)
...
// =========================================================
// setbutton — Button Mode true/false
// =========================================================
cmd({
    pattern: "setbutton",
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, sessionNumber, setUserConfigInMongo }) => {
    try {
        ...
        await inputSession(sessionNumber, "BUTTON", val, setUserConfigInMongo)
...
// =========================================================
// setownerreact — Owner React true/false
// =========================================================
cmd({
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, sessionNumber, setUserConfigInMongo }) => {
    try {
        ...
        await inputSession(sessionNumber, "OWNER_REACT", val, setUserConfigInMongo)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, sessionNumber, setUserConfigInMongo }) => {
    try {
        ...
        await inputSession(sessionNumber, "WORK_TYPE", q.trim().toLowerCase(), setUserConfigInMongo)
...
async (conn, mek, m, { q, isOwner, senderNumber, botNumber, reply, sessionNumber, setUserConfigInMongo }) => {
    try {
        if (!isAllowed(isOwner, senderNumber, botNumber))
            return reply("*⛔ Only the bot owner can use this command!*")
        await inputSession(sessionNumber, "PREFIX", q, setUserConfigInMongo)
        reply(`*✅ New Prefix:* ${q}`)
    } catch (e) {
        console.log(e)
    }
