...
//---------------- FB DOWNLOAD HELPER (movanest API) ------------------//
async function getFBData(url) {
    try {
        const res = await axios.get("https://www.movanest.xyz/v2/fbdown", { params: { url } });
        if (!res.data.status || !res.data.results?.[0]) return { status: false };
...
//---------------- TIKTOK DOWNLOAD HELPER (movanest API) ------------------//
async function getTikTokData(url) {
    try {
        const res = await axios.get("https://www.movanest.xyz/v2/tiktok", { params: { url } });
        if (!res.data.status || !res.data.results) return null;
        const r = res.data.results;
        return {
            title: r.desc || "TikTok Video",
            regions: r.author_nickname || "Unknown",
            runtime: "N/A",
            thumbnail: r.cover || r.thumbnail || "",
            no_watermark: r.no_watermark || "",
...
const mov = await getTikTokData(q);
if (!mov) return await reply(N_FOUND);
