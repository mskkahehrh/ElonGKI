...
const { connectdb, updb, getConfig } = require('./lib/database');
...
function setupCommandHandlers(socket, number) {
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      // =============================
      // 🔒 WORK TYPE FILTER (dynamic — updated by .setwork, per-user)
      // =============================
      const userCfg = getConfig(botNumber);
      const workType = (userCfg.WORK_TYPE || 'private').toLowerCase();
      if (workType === 'private') {
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        try {
          ...
          // Load this user's settings into the per-user config cache
          try { await updb(sanitizedNumber); } catch (e) { console.warn('updb on connect failed:', e); }
...
(async () => {
  try {
    await connectdb();  // mongoose settings connect (per-user configs loaded on session open)
