...
// WORK_TYPE is read dynamically from config (updated by updb()) - do not cache as static const
...
async function setupStatusHandlers(socket, sessionNumber) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
      // AUTO_VIEW_STATUS and AUTO_LIKE_STATUS are global settings managed by
      // setautoview / setautolike commands (stored in mongoose Settings, synced
      // into config via updb()). Do NOT override them from the per-user configsCol
      // because setautoview/setautolike only update the global Settings — if we let
      // the per-user config override here, those commands stop working.
      const autoViewStatus = config.AUTO_VIEW_STATUS; // Always read from global config
      const autoLikeStatus = config.AUTO_LIKE_STATUS; // Always read from global config

      // Per-user config is only used for emoji customisation and AUTO_RECORDING,
      // which can legitimately differ per session.
      ...
      if (sessionNumber) {
        ...
        // Only override emojis from per-user config
        if (userConfig.AUTO_LIKE_EMOJI && Array.isArray(userConfig.AUTO_LIKE_EMOJI) && userConfig.AUTO_LIKE_EMOJI.length > 0) {
          userEmojis = userConfig.AUTO_LIKE_EMOJI;
        }

        // Allow per-user recording override
      ...
      // AUTO VIEW / AUTO LIKE — WhatsApp requires a status to be viewed
      // (readMessages) before a reaction can be sent. So if LIKE is enabled
      // we must view first, even when VIEW alone is off.
      const shouldView = autoViewStatus === 'true' || autoLikeStatus === 'true';
      if (shouldView) {
        let retries = config.MAX_RETRIES;
        while (retries > 0) {
          try {
...
function setupCommandHandlers(socket, number) {
  ...
  // Each session gets its own config copy so that setting changes made by
  // one user never bleed into another user's session.
  const sessionConfig = Object.assign({}, config);

  // Load any previously saved settings for this session from the DB and
  // merge them into the session-local config object.
  updb(sanitizedNumber).then(dbSettings => {
    if (dbSettings) Object.assign(sessionConfig, dbSettings);
  }).catch(() => {});
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      const cleanOwner = (sessionConfig.OWNER_NUMBER || config.OWNER_NUMBER).replace(/[^0-9]/g, '');
      const cleanBot   = botNumber.replace(/[^0-9]/g, '');
      const isOwner = cleanSender === cleanOwner || cleanSender === cleanBot;

      // =============================
      // 🔒 WORK TYPE FILTER (uses this session's own config)
      // =============================
      const workType = (sessionConfig.WORK_TYPE || 'private').toLowerCase();
      if (workType === 'private') {
      ...
      await cmdData.function(conn, mek, null, {
        ...
        pushname,
        reply,
        // Pass this session's own config object — changes made here only
        // affect this session, not any other connected user.
        config: sessionConfig,
        sessionNumber: sanitizedNumber
      });
...
(async () => {
  try {
    ...
    // Load the global/default settings into the shared config object for
    // code that still reads from it (e.g. IMAGE_PATH, BOT_NAME, etc.).
    // Per-user settings are loaded into each session's own sessionConfig
    // inside setupCommandHandlers — they never mutate this global object.
    const globalSettings = await updb('global');
    Object.assign(config, globalSettings);
