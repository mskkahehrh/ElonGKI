...
async function setupStatusHandlers(socket, sessionNumber) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
      ...
      // Per-user config for status features (loaded by sessionNumber = bot's linked number)
      const _statusUserCfg = sessionNumber ? (await loadUserConfigFromMongo(sessionNumber) || {}) : {};
      const autoViewStatus = _statusUserCfg.AUTO_VIEW_STATUS !== undefined ? _statusUserCfg.AUTO_VIEW_STATUS : config.AUTO_VIEW_STATUS;
      const autoLikeStatus = _statusUserCfg.AUTO_LIKE_STATUS !== undefined ? _statusUserCfg.AUTO_LIKE_STATUS : config.AUTO_LIKE_STATUS;

      // Per-user config already loaded above as _statusUserCfg
      let userEmojis = config.AUTO_LIKE_EMOJI;
      let autoRecording = config.AUTO_RECORDING;

      if (_statusUserCfg.AUTO_LIKE_EMOJI && Array.isArray(_statusUserCfg.AUTO_LIKE_EMOJI) && _statusUserCfg.AUTO_LIKE_EMOJI.length > 0) {
        userEmojis = _statusUserCfg.AUTO_LIKE_EMOJI;
      }
      if (_statusUserCfg.AUTO_RECORDING !== undefined) {
        autoRecording = _statusUserCfg.AUTO_RECORDING;
      }
...
function setupCommandHandlers(socket, number) {
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      // =============================
      // 🔒 WORK TYPE FILTER (dynamic — updated by .setwork)
      // =============================
      const _sessionCfg = await loadUserConfigFromMongo(cleanBot) || {};
      const workType = (_sessionCfg.WORK_TYPE || config.WORK_TYPE || 'private').toLowerCase();
      if (workType === 'private') {
        // private: only owner / linked number
        if (!msg.key.fromMe && !isOwner) return;
      } else if (workType === 'group') {
        // group: only groups (owner allowed in DM too)
        if (!isGroup && !isOwner) return;
      } else if (workType === 'inbox') {
        // inbox: only DMs (owner allowed everywhere)
        if (isGroup && !isOwner) return;
      }
      ...
      const prefix = _sessionCfg.PREFIX || config.PREFIX;
      ...
      // Merge per-user config on top of global config so plugins see per-device settings
      const _mergedCfg = Object.assign({}, config, _sessionCfg);
      await cmdData.function(conn, mek, null, {
        from,
        prefix,
        body,
...
async function EmpirePair(number, res) {
  ...
  try {
    ...
    const ns = initNumberSystem({
      conn: socket,
      mongoDB,
      PREFIX: _sessionCfg.PREFIX || config.PREFIX
    });
