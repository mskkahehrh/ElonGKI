...
const { connectdb, updb, getUserConfig } = require('./lib/database');
...
async function setupStatusHandlers(socket, sessionNumber) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
      // Load per-user config so each user's auto-view/like settings are independent
      const perUserCfg = sessionNumber
        ? await getUserConfig(sessionNumber).catch(() => config)
        : config;

      const autoViewStatus = perUserCfg.AUTO_VIEW_STATUS || config.AUTO_VIEW_STATUS;
      const autoLikeStatus = perUserCfg.AUTO_LIKE_STATUS || config.AUTO_LIKE_STATUS;
      let autoRecording = perUserCfg.AUTO_RECORDING || config.AUTO_RECORDING;

      // Per-user emoji list (falls back to global)
      let userEmojis = config.AUTO_LIKE_EMOJI;
      if (sessionNumber) {
        const legacyUserConfig = await loadUserConfigFromMongo(sessionNumber).catch(() => ({})) || {};
        if (legacyUserConfig.AUTO_LIKE_EMOJI && Array.isArray(legacyUserConfig.AUTO_LIKE_EMOJI) && legacyUserConfig.AUTO_LIKE_EMOJI.length > 0) {
          userEmojis = legacyUserConfig.AUTO_LIKE_EMOJI;
        }
      }
...
function setupCommandHandlers(socket, number) {
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      // Load per-user config for this session so settings commands
      // only affect the user who ran them (not all connected users)
      const userCfg = await getUserConfig(botNumber).catch(() => config);

      // =============================
      // 🔒 WORK TYPE FILTER (per-user — updated by .setwork)
      // =============================
