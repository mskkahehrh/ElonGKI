...
async function setupStatusHandlers(socket, sessionNumber) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
      // Load per-session config — each bot has its own isolated settings
      const { getSessionConfig } = require('./lib/database');
      const sc = await getSessionConfig(sessionNumber);

      const autoViewStatus = sc.AUTO_VIEW_STATUS !== undefined ? sc.AUTO_VIEW_STATUS : config.AUTO_VIEW_STATUS;
      const autoLikeStatus = sc.AUTO_LIKE_STATUS !== undefined ? sc.AUTO_LIKE_STATUS : config.AUTO_LIKE_STATUS;
      let autoRecording    = sc.AUTO_RECORDING   !== undefined ? sc.AUTO_RECORDING   : config.AUTO_RECORDING;

      let userEmojis = (sc.AUTO_LIKE_EMOJI && Array.isArray(sc.AUTO_LIKE_EMOJI) && sc.AUTO_LIKE_EMOJI.length > 0)
        ? sc.AUTO_LIKE_EMOJI
...
function setupCommandHandlers(socket, number) {
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      // =============================
      // 🔒 WORK TYPE FILTER (per-session — each bot has its own WORK_TYPE)
      // =============================
      const { getSessionConfig } = require('./lib/database');
      const _sc = await getSessionConfig(session);
      const workType = (_sc.WORK_TYPE || config.WORK_TYPE || 'private').toLowerCase();
      if (workType === 'private') {
      ...
      const prefix = _sc.PREFIX || config.PREFIX;
      ...
      // Merge per-session config on top of global config so plugins see correct values
      const mergedConfig = Object.assign({}, config, _sc);
...
async function setupCallRejection(socket, sessionNumber) {
    socket.ev.on('call', async (calls) => {
        try {
            ...
            const { getSessionConfig: _gcsc } = require('./lib/database');
            const userConfig = await _gcsc(sanitized);
...
async function setupAutoMessageRead(socket, sessionNumber) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    const sanitized = (sessionNumber || '').replace(/[^0-9]/g, '');
    const { getSessionConfig: _gsc } = require('./lib/database');
    const userConfig = await _gsc(sanitized);
...
async function setupMessageHandlers(socket, sessionNumber) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      const sanitized = (sessionNumber || '').replace(/[^0-9]/g, '');
      const { getSessionConfig: _getSC } = require('./lib/database');
      const userConfig = await _getSC(sanitized);
